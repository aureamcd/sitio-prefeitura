...
export const TABELAS_TRANSPARENCIA: TableConfig[] = [
...

  {
    ...
    formFields: [
      ...
      { key: "empresa", label: "Código da Entidade" },
      { key: "empresa_nome", label: "Nome da Entidade" },
...

  {
    slug: "emendas",
    label: "Emendas",
    description: "Emendas parlamentares recebidas pelo município (com indicação de parlamentar, objeto e valores)",
    icon: Landmark,
    color: "text-rose-600",
    bgColor: "bg-rose-50",
    schema: "transparencia",
    ...
    orderBy: { column: "ano", ascending: false },
    columns: [
      { key: "parlamentar", label: "Parlamentar", width: "min-w-[160px]" },
      { key: "numero_emenda", label: "Nº Emenda/Proposta", hideMobile: true },
      { key: "objeto", label: "Objeto do Gasto", width: "min-w-[220px]" },
      { key: "valor_previsto", label: "Valor Previsto", type: "number", monetary: true },
      { key: "ano", label: "Ano", width: "w-16" },
    ],
    mobileColumns: ["parlamentar", "valor_previsto", "ano"],
    formFields: [
      { key: "parlamentar", label: "Parlamentar / Autor da Emenda", required: true },
      { key: "numero_emenda", label: "Número da Emenda / Proposta / Plano de Ação" },
      { key: "objeto", label: "Objeto do Gasto / Descrição", type: "textarea", required: true },
      { key: "beneficiario", label: "Órgão / Fundo Beneficiário" },
      { key: "valor_previsto", label: "Valor Previsto (R$)", type: "number", required: true },
      { key: "ano", label: "Ano", type: "number", required: true },
    ],
    filters: [
      { key: "ano", label: "Ano", getOptions: (items) => getUniqueYears(items).map(a => ({ value: String(a), label: String(a) })) },
...

  {
    ...
    columns: [
      { key: "objeto", label: "Objeto", width: "min-w-[250px]" },
      { key: "contrato_numero", label: "Contrato", width: "w-24", hideMobile: true },
      { key: "localizacao", label: "Local", hideMobile: true },
      { key: "situacao", label: "Situação" },
      { key: "empresa_responsavel", label: "Empresa", hideMobile: true },
    ...
    formFields: [
      ...
      { key: "situacao", label: "Situação" },
      { key: "contrato_numero", label: "Nº do Contrato" },
      { key: "licitacao", label: "Licitação (Modalidade/Nº)", placeholder: "ex: CONCORRÊNCIA Nº 013/2025" },
      ...
      { key: "valor_total", label: "Valor Total Contratado", type: "number" },
  ...
  },
  {
    ...
    description: "PCA — Planejamento anual de contratações do município",
    icon: FileText,
    color: "text-sky-600",
    bgColor: "bg-sky-50",
    schema: "transparencia",
    table: "plano_contratacoes_anual",
    orderBy: { column: "ano", ascending: false },
    columns: [
      { key: "ano", label: "Ano", width: "w-16" },
      { key: "responsavel", label: "Responsável", width: "min-w-[180px]" },
      { key: "cargo_responsavel", label: "Cargo", hideMobile: true },
      { key: "frequencia_atualizacao", label: "Frequência", hideMobile: true },
      { key: "data_publicacao", label: "Data Publicação", type: "date", hideMobile: true },
    ],
    mobileColumns: ["ano", "responsavel"],
    formFields: [
      { key: "ano", label: "Ano", type: "number", required: true },
      { key: "arquivo_url", label: "Link do Arquivo (PDF/Planilha)", fullWidth: true, placeholder: "URL do arquivo no R2 ou Google Drive" },
      { key: "arquivo_nome", label: "Nome do Arquivo", placeholder: "ex: PCA_2027.xlsx" },
      { key: "responsavel", label: "Responsável pelo PCA", required: true, placeholder: "Nome do servidor responsável" },
      { key: "cargo_responsavel", label: "Cargo do Responsável" },
      { key: "frequencia_atualizacao", label: "Frequência de Atualização", placeholder: "ex: Anual, Semestral" },
      { key: "data_publicacao", label: "Data de Publicação", type: "date" },
    ],
    filters: [
      { key: "ano", label: "Ano", getOptions: (items) => getUniqueYears(items).map(a => ({ value: String(a), label: String(a) })) },
    ],
...

  {
    ...
    description: "Ordem cronológica de pagamentos — Critério 9.4 do PNTP 2026",
    icon: Clock,
    color: "text-teal-600",
    bgColor: "bg-teal-50",
    schema: "transparencia",
    table: "ordem_cronologica_pagamentos",
    orderBy: { column: "data_pagamento", ascending: false },
    columns: [
      { key: "fornecedor", label: "Fornecedor", width: "min-w-[200px]" },
      { key: "valor_pago", label: "Valor Pago", type: "number", monetary: true },
      { key: "data_pagamento", label: "Data Pagamento", type: "date", hideMobile: true },
      { key: "data_vencimento", label: "Data Vencimento", type: "date", hideMobile: true },
      { key: "empenho", label: "Empenho", hideMobile: true },
      { key: "ano", label: "Ano", width: "w-16" },
    ],
    mobileColumns: ["fornecedor", "valor_pago", "ano"],
    formFields: [
      { key: "ano", label: "Ano", type: "number", required: true },
      { key: "fornecedor", label: "Fornecedor", required: true },
      { key: "cpf_cnpj_fornecedor", label: "CPF/CNPJ" },
      { key: "valor_empenho", label: "Valor Empenhado", type: "number" },
      { key: "valor_pago", label: "Valor Pago", type: "number" },
      { key: "valor_desconto", label: "Valor Desconto", type: "number" },
      { key: "valor_liquido_pago", label: "Valor Líquido Pago", type: "number" },
      ...
      { key: "historico", label: "Histórico", type: "textarea", fullWidth: true },
      { key: "justificativa", label: "Justificativa (código)" },
      { key: "justificativa_texto", label: "Justificativa (texto)", type: "textarea", fullWidth: true },
      { key: "data_justificativa", label: "Data Justificativa", type: "date" },
      { key: "notas_fiscais", label: "Notas Fiscais" },
      { key: "empresa_nome", label: "Entidade" },
  ...
  {
    slug: "sancionados",
    label: "Licitantes Sancionados",
    description: "Empresas e licitantes com sanções administrativas",
    icon: Gavel,
    color: "text-red-600",
    bgColor: "bg-red-50",
    schema: "transparencia",
    ...
    columns: [
      { key: "ano", label: "Ano", width: "w-16" },
      { key: "empresa_nome", label: "Empresa", width: "min-w-[200px]" },
      { key: "cnpj", label: "CNPJ" },
      { key: "tipo_sancao", label: "Tipo Sanção", hideMobile: true },
      { key: "data_inicio", label: "Data Início", type: "date", hideMobile: true },
    ],
    mobileColumns: ["ano", "empresa_nome", "tipo_sancao"],
    formFields: [
      { key: "ano", label: "Ano", type: "number", required: true },
      { key: "empresa_nome", label: "Nome da Empresa", required: true },
      { key: "cnpj", label: "CPF/CNPJ" },
      { key: "tipo_sancao", label: "Tipo de Sanção", required: true, placeholder: "ex: Suspensão, Declaração de Inidoneidade, Multa" },
      { key: "data_inicio", label: "Data Início", type: "date" },
      { key: "data_fim", label: "Data Fim", type: "date" },
      { key: "processo", label: "Nº do Processo" },
      { key: "motivo", label: "Motivo", type: "textarea", fullWidth: true },
      { key: "observacoes", label: "Observações", type: "textarea", fullWidth: true },
    ],
    filters: [
      { key: "ano", label: "Ano", getOptions: (items) => getUniqueYears(items).map(a => ({ value: String(a), label: String(a) })) },
      { key: "tipo_sancao", label: "Tipo", getOptions: (items) => getUniqueValues(items, "tipo_sancao").map(v => ({ value: v, label: v })) },
    ],
  },
