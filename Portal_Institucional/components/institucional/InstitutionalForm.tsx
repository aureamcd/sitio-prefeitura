 ...
 * Em vez de criar 2 formulários separados, usamos um
 * componente genérico que recebe a configuração dos campos
 * como prop. Assim, a lógica de envio, validação, loading,
 * sucesso e protocolo é escrita UMA VEZ e reutilizada.
 *
...
export default function InstitutionalForm({
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      let res;
      if (hasFile) {
        /* Se tem arquivo, envia como multipart/form-data (padrão do FormData) */
        res = await fetch(apiUrl, {
          method: "POST",
          body: formData,
        });
      } else {
        /* Caso contrário, continua enviando JSON (para manter compatibilidade) */
      ...
      /* Sucesso: exibe a tela de protocolo */
  ...
  /* ── Se já enviou, exibe a tela de sucesso ── */
  if (protocolo) {
    return (
      <ProtocolSuccess
        protocolo={protocolo}
