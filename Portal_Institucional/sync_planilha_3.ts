...
async function run() {
  ...
  const { data: dbRecords, error } = await supabase
    ...
    .from("licitacoes_v2")
    .select("id, proclic, link_tce, objeto, numero, processo, ano");
  ...
  for (const r of dbRecords) {
    ...
    if (r.objeto) mapByObjeto.set(r.objeto.trim().substring(0, 100).toLowerCase(), r); // First 100 chars
  }
  ...
  console.log(`Registros atualizados com sucesso (passo 3): ${updatedCount}`);
  console.log(`Não encontrados no banco (passo 3): ${notFound}`);
}
