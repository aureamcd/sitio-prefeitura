...
export default function AdminPublicacoesPage() {
  ...
  async function handleDelete() {
    ...
    if (error) {
      ...
      showToast("success", "Publicação excluída com sucesso.");
    }
    setDeleteId(null);
    setDeleting(false);
  }
  ...
  async function handleBulkUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const res = await fetch("/api/admin/publicacoes/upload", {
          method: "POST",
          body: formData,
        });
        if (!res.ok) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-gray-900">Publicações Oficiais</h1>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {paged.map(item => (
                  <tr
                      key={item.id}
                      onClick={() => router.push(`/admin/publicacoes/${item.id}/editar`)}
