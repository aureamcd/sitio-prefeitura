...
export default async function EditarPublicacaoPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (error || !row) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4 text-gray-700">
        ...
        <p className="text-lg font-bold text-gray-800">Publicação não encontrada</p>
  ...
  const data = {
    id:              row.id,
    titulo:          row.titulo          ?? "",
    tipo:            row.tipo            ?? row.categoria ?? "Boletim",
    numero:          row.numero          ?? "",
    ano:             row.ano             ? Number(row.ano) : new Date().getFullYear(),
    descricao:       row.descricao       ?? "",
    orgao:           row.orgao           ?? "",
    data_publicacao: row.data_publicacao ?? "",
    publicado:       row.publicado       ?? false,
    arquivo_r2_url:  row.arquivo_r2_url  ?? "",
    arquivo_nome:    row.arquivo_nome    ?? "",
  };

  return <DocumentoForm mode="editar" initialData={data} categoriaInicial="publicacoes-oficiais" />;
}
