...
import {
  ...
  CheckCircle2, XCircle, RefreshCw, X, FileUp, Upload
} from "lucide-react";

const TIPOS = [
  "Ofício", "Edital", "Ata", "Notificação", "Comunicado", 
  "Nota", "Calendário", "Despacho", "Questionário", 
  "Boletim", "Protocolo", "Requerimento", "Outros"
];
...
export default function AdminPrestacaoContasPage() {
  ...
  const [bulkTipo, setBulkTipo] = useState("Edital");
  ...
  async function fetchAll() {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/prestacao-contas");
      if (!res.ok) {
  ...
  const anos = useMemo(
    () => [...new Set(items.map(i => i.ano).filter(Boolean) as string[])].sort((a, b) => b.localeCompare(a)),
    [items]
  );
  ...
  const filtered = useMemo(() => {
    ...
    // Função para extrair timestamp real para ordenação correta (DD/MM/YYYY ou ISO ou ano)
    const getTimestamp = (doc: Pub) => {
      if (doc.data_publicacao) {
        if (doc.data_publicacao.includes("/")) {
          const parts = doc.data_publicacao.split("/");
          if (parts.length === 3) return new Date(parts.reverse().join("-")).getTime();
        }
        return new Date(doc.data_publicacao).getTime();
      }
      if (doc.ano) return new Date(`${doc.ano}-01-01`).getTime();
      return new Date(doc.created_at || 0).getTime();
    };

    return items
      .filter(i => {
        if (filtroTipo && i.tipo !== filtroTipo) return false;
        if (filtroAno && i.ano !== filtroAno) return false;
  ...
  }, [items, busca, filtroTipo, filtroAno]);
  ...
  async function handleDelete() {
    if (!deleteId) return;
    setDeleting(true);
    const { error } = await supabase.from("publicacoes").delete().eq("id", deleteId);
    if (error) {
      showToast("error", "Erro ao excluir: " + error.message);
    } else {
      setItems(prev => prev.filter(i => i.id !== deleteId));
      showToast("success", "Prestação de Contas excluída com sucesso.");
    }
    setDeleteId(null);
    setDeleting(false);
  }
  ...
  async function handleBulkUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const res = await fetch("/api/admin/prestacao-contas/upload", {
          method: "POST",
          body: formData,
        });
        if (!res.ok) {
  ...
  function fmtData(iso: string | null) {
    if (!iso) return "—";
    if (iso.includes("/")) return iso; // Já está formatado DD/MM/YYYY
  ...
  const TIPO_COLOR: Record<string, string> = {
    ...
    "Comunicado": "bg-teal-700 text-white",
    "Notificação": "bg-indigo-100 text-indigo-700",
    "Boletim": "bg-cyan-100 text-cyan-700",
    "Protocolo": "bg-rose-100 text-rose-700",
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-gray-900">Prestação de Contas Oficiais</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link
            href="/admin/prestacao-contas/nova"
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 space-y-3">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          {/* Limpar */}
          {(busca || filtroTipo || filtroAno) && (
            <button
              onClick={() => { setBusca(""); setFiltroTipo(""); setFiltroAno(""); setPage(1); }}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-3 text-gray-600">
            ...
            <p className="text-sm font-medium">Nenhum arquivo encontrada</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-bold text-gray-700 uppercase tracking-wider hidden lg:table-cell">Data</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {paged.map(item => (
                  <tr
                      key={item.id}
                      onClick={() => router.push(`/admin/prestacao-contas/${item.id}/editar`)}
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-gray-700 font-medium">
                          {item.numero ? `Nº ${item.numero}/${item.ano ?? ""}` : `S/N${item.ano ? `/${item.ano}` : ""}`}
                        </p>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {(item.arquivo_r2_url || item.arquivo_url) ? (
                        <a
                          href={item.arquivo_r2_url || item.arquivo_url || "#"}
                          target="_blank"
                    ...
                    <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          href={`/admin/prestacao-contas/${item.id}/editar`}
