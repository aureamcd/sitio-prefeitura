...
import { ChevronRight, ChevronDown, Receipt, AlertCircle } from 'lucide-react';
...
// ---------------------------------------------------------------------------
// Dashboard Summary (idêntico ao de 2026)
// ---------------------------------------------------------------------------
...
function DashboardSummary({
  ...
  totalPrevistoAtualizado: number;
  totalArrecadadoPeriodo: number;
  loading: boolean;
}) {
  const totalPct =
    totalPrevistoAtualizado > 0
      ? (totalArrecadadoPeriodo / totalPrevistoAtualizado) * 100
      : 0;
  return (
    <div className="mt-4 mb-4 mx-auto bg-white border border-blue-100 rounded-2xl p-4 shadow-sm flex flex-col items-center text-center relative overflow-hidden">
      ...
      <div className="flex flex-wrap justify-center gap-4 sm:gap-8 w-full relative z-10">
        <div className="flex flex-col items-center p-2 rounded-xl hover:bg-gray-50 transition-colors">
          ...
          {loading ? <div className="h-6 w-24 bg-gray-100 animate-pulse rounded mt-1" /> : <p className="text-xl sm:text-2xl font-extrabold text-slate-700 tabular-nums">{formatBRL(totalPrevistoInicial)}</p>}
        </div>
        ...
        <div className="flex flex-col items-center p-2 rounded-xl hover:bg-gray-50 transition-colors">
          ...
          {loading ? <div className="h-6 w-24 bg-gray-100 animate-pulse rounded mt-1" /> : <p className="text-xl sm:text-2xl font-extrabold text-slate-700 tabular-nums">{formatBRL(totalPrevistoAtualizado)}</p>}
        </div>
        <div className="hidden sm:block w-px bg-gradient-to-b from-transparent via-blue-200 to-transparent" />
        <div className="flex flex-col items-center p-2 rounded-xl hover:bg-blue-50 transition-colors">
          ...
          {loading ? <div className="h-6 w-24 bg-blue-100 animate-pulse rounded mt-1" /> : <p className="text-xl sm:text-2xl font-extrabold text-blue-700 tabular-nums">{formatBRL(totalArrecadadoPeriodo)}</p>}
        </div>
        ...
        <div className="flex flex-col items-center p-2 rounded-xl hover:bg-emerald-50 transition-colors">
          ...
          {loading ? <div className="h-6 w-12 bg-emerald-100 animate-pulse rounded mt-1" /> : <p className="text-xl sm:text-2xl font-extrabold text-emerald-600 tabular-nums">{totalPct.toFixed(1)}%</p>}
        </div>
      </div>
    </div>
  );
...
// ---------------------------------------------------------------------------
// TreeNode — visual idêntico ao Row do TreeTable de 2026
// ---------------------------------------------------------------------------
...
const TreeNode = React.memo(
  ({
    ...
    item: HistoricoReceita;
    childrenMap: Map<string, HistoricoReceita[]>;
  }) => {
    ...
    const levelLabel = item.nome_nivel || getLevelName(undefined, logicalDepth);
...
export default function HistoricoTable({ ano, entidade }: HistoricoTableProps) {
  const [data, setData] = useState<HistoricoReceita[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        else if (result) { setData(result as HistoricoReceita[]); }
        else { setData([]); }
      } catch (err) {
  ...
  // --- Build tree (roots + childrenMap) com Regra de Ouro ---
  const { roots, childrenMap } = useMemo(() => {
    const map = new Map<string, HistoricoReceita[]>();
    data.forEach((item) => {
      let pai = item.codigo_pai || 'ROOT';

      if (item.is_analitica) {
        if (data.some(d => !d.is_analitica && d.codigo_contabil === item.codigo_contabil)) {
          pai = item.codigo_contabil;
        }
      }

      if (!map.has(pai)) map.set(pai, []);
      map.get(pai)!.push(item);
    });

    const availableCodes = new Set(data.map(d => d.codigo_contabil));
    const rootsList: HistoricoReceita[] = [];
    data.forEach(item => {
  ...
  // --- Dashboard totals (soma dos níveis 1) ---
  const totals = useMemo(() => {
    const nivel1Items = data.filter(item => item.nivel === 1);
    return nivel1Items.reduce(
      (acc, root) => ({
      ...
      { totalPrevistoInicial: 0, totalPrevistoAtualizado: 0, totalArrecadadoPeriodo: 0 }
    );
  }, [data]);

  // --- Pagination ---
  const filterKey = `${ano}-${entidade}`;
  const pagination = usePagination(roots, PAGE_SIZE, filterKey);

  return (
    <div className="space-y-4">
      <DashboardSummary
        totalPrevistoInicial={totals.totalPrevistoInicial}
        totalPrevistoAtualizado={totals.totalPrevistoAtualizado}
        totalArrecadadoPeriodo={totals.totalArrecadadoPeriodo}
        loading={loading}
      />
