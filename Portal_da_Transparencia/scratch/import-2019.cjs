...
const DESKTOP_FOLDER = "C:\\Users\\Áurea Letícia\\Desktop\\contratos\\Contratos\\2019";
...
async function run() {
  try {
      ...
      console.log("Baixando contratos e documentos do banco de dados para 2019...");
      
      const { data: contratos } = await supabase.schema('transparencia').from('contratos_v2').select('id, numero, processo').eq('ano', 2019);
      ...
      console.log(`Encontrados ${filesPaths.length} arquivos PDF na pasta 2019.`);
      ...
      for (const filePath of filesPaths) {
        ...
        try {
            ...
            const { error: insErr } = await supabase.schema('transparencia').from('contratos_documentos').insert({
                ...
                caminho_r2: r2Key,
                origem: 'importacao-desktop-2019'
            });
      ...
      console.log("\n=================================");
      console.log("RESUMO DA MIGRAÇÃO (PASTA 2019)");
      console.log("=================================");
...
function findMatch(cleanNum, contratos) {
    ...
    // Tentar sem o ano (ex: 051-2019 tenta achar 051)
...
function identifyContract(filename, contratos) {
    ...
    // Regra para capturar "CONTRATO Nº 2019-002.pdf" -> Inverter para 002-2019
