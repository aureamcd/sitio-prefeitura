...
const DESKTOP_FOLDER = "C:\\Users\\Áurea Letícia\\Desktop\\contratos\\Contratos\\2018";
...
async function run() {
  try {
      ...
      console.log("Baixando contratos e documentos do banco de dados para 2018...");
      
      const { data: contratos } = await supabase.schema('transparencia').from('contratos_v2').select('id, numero, processo').eq('ano', 2018);
      ...
      console.log(`Encontrados ${filesPaths.length} arquivos PDF na pasta 2018.`);
      ...
      for (const filePath of filesPaths) {
        ...
        try {
            ...
            const { error: insErr } = await supabase.schema('transparencia').from('contratos_documentos').insert({
                ...
                caminho_r2: r2Key,
                origem: 'importacao-desktop-2018'
            });
      ...
      console.log("\n=================================");
      console.log("RESUMO DA MIGRAÇÃO (PASTA 2018)");
      console.log("=================================");
...
function findMatch(cleanNum, contratos) {
    ...
    // Tentar sem o ano (ex: 051-2018 tenta achar 051)
...
function identifyContract(filename, contratos) {
    ...
    // "CONTRATO 051-2018" ou "CONT 038"
    const regex1 = /(?:CONTRATO(?:S)?|CONT)\s+(?:ADM(?:INISTRATIVO)?\s+)?(?:N[º°]?\s*)?([0-9A-Z\-\/]+)/;
    ...
    // Regra para capturar "CONTRATO Nº 2018-002.pdf" -> Inverter para 002-2018
