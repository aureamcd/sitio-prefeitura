...
const DESKTOP_FOLDER = "C:\\Users\\Áurea Letícia\\Desktop\\contratos\\Contratos\\2021";
...
async function run() {
  try {
      ...
      console.log("Baixando contratos e documentos do banco de dados para 2021...");
      
      const { data: contratos } = await supabase.schema('transparencia').from('contratos_v2').select('id, numero, processo').eq('ano', 2021);
      ...
      console.log(`Encontrados ${filesPaths.length} arquivos PDF na pasta 2021.`);
      ...
      for (const filePath of filesPaths) {
        ...
        try {
            ...
            const { error: insErr } = await supabase.schema('transparencia').from('contratos_documentos').insert({
                ...
                caminho_r2: r2Key,
                origem: 'importacao-desktop-2021'
            });
      ...
      console.log("\n=================================");
      console.log("RESUMO DA MIGRAÇÃO (PASTA 2021)");
      console.log("=================================");
...
function findMatch(cleanNum, contratos) {
    ...
    // Tentar sem o ano (ex: 051-2021 tenta achar 051)
