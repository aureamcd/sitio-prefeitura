...
// Retorna se dois contratos são variações do mesmo (ex: 058, 058/2019, Termo de contrato 058/2019)
function isSameContract(base, num1, num2) {
  const normalize = (s) => s.toLowerCase().replace(/termo de contrato/g, '').replace(/\/2019/g, '').trim();
  const n1 = normalize(num1);
  const n2 = normalize(num2);
  
  // Se após remover "Termo..." e "/2019" eles ficarem iguais (ex: "058" e "058")
...
async function run() {
  console.log("Iniciando mesclagem inteligente 2019...");
  const { data: contratos } = await supabase.schema('transparencia').from('contratos_v2').select('*, documentos:contratos_documentos(*)').eq('ano', 2019);
