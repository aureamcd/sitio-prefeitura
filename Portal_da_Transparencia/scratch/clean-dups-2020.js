...
async function run() {
    console.log("Iniciando exclusão de PDFs duplicados de 2020 (Priorizando a Pasta Local)...");
    
    const { data: contratos } = await supabase.schema('transparencia').from('contratos_v2').select('id, numero').eq('ano', 2020);
        ...
        if (docs.length > 1) {
            ...
            for (const size in bySize) {
                ...
                if (dupGroup.length > 1) {
                    // ORDENAÇÃO CUSTOMIZADA PARA 2020: 
                    // Queremos priorizar (manter no topo = index 0) o arquivo cuja origem seja 'importacao-desktop-2020'
                    dupGroup.sort((a, b) => {
                        const aIsRecent = a.origem === 'importacao-desktop-2020';
                        const bIsRecent = b.origem === 'importacao-desktop-2020';
