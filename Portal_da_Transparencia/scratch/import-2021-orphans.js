...
const DIR = 'C:\\Users\\Áurea Letícia\\Desktop\\contratos\\Contratos\\2021';
...
function extractNumber(name) {
  ...
  // Pegar números no começo: "35-2021...", "018-A-2021...", "40 A - 2021"
  let m = name.match(/^0*(\d+)(?:\s*-?\s*[a-z])?(?:\s*-?\s*2021)/i);
  if (m) return m[1];
  
  // "CONTRATO Nº 05-2021" -> 5
  m = name.match(/Nº\s*0*(\d+)-2021/i);
  if (m) return m[1];
...
async function run() {
  ...
  const { data: contratos } = await supabase.schema('transparencia').from('contratos_v2').select('*').eq('ano', 2021);
  ...
  for (const file of files) {
    ...
    if (num) {
      ...
      if (possible.length === 1) {
        ...
        const caminhoR2 = `contratos/2021/${fileNameToSave}`;
