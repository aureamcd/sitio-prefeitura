...
async function run() {
    console.log("Iniciando exclusão de PDFs duplicados de 2019 (Priorizando a Pasta Local)...");
    
    const { data: contratos } = await supabase.schema('transparencia').from('contratos_v2').select('id, numero').eq('ano', 2019);
        ...
        if (docs.length > 1) {
            ...
            for (const size in bySize) {
                ...
                if (dupGroup.length > 1) {
                    // ORDENAÇÃO CUSTOMIZADA PARA 2019: 
                    // Queremos priorizar (manter no topo = index 0) o arquivo cuja origem seja 'importacao-desktop-2019'
                    dupGroup.sort((a, b) => {
                        const aIsRecent = a.origem === 'importacao-desktop-2019';
                        const bIsRecent = b.origem === 'importacao-desktop-2019';
