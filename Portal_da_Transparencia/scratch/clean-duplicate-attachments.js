...
async function run() {
    console.log("Iniciando exclusão de PDFs duplicados de 2017...");
        ...
        if (docs.length > 1) {
            ...
            for (const size in bySize) {
                ...
                if (dupGroup.length > 1) {
                    // Ordenar por ID para ser determinístico, manter o mais antigo (menor ID)
                    // Ou o que tem um nome "melhor". Vamos manter o primeiro da lista ordenada.
                    dupGroup.sort((a, b) => a.id.localeCompare(b.id));
                    
                    const survivor = dupGroup[0];
                    const toDelete = dupGroup.slice(1);
                    
                    console.log(`\nLimpando contrato ${contractId} (Tamanho: ${size} bytes)`);
                    console.log(`  🟢 MANTENDO: ${survivor.nome_arquivo}`);

                    for (const docDel of toDelete) {
                        console.log(`  🔴 DELETANDO: ${docDel.nome_arquivo}`);
