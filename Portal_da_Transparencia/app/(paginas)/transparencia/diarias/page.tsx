...
export default function DiariasPage() {
  ...
  const valorTotal = data.reduce((acc, curr) => acc + (Number(curr.valor) || 0) - (Number(curr.valor_anulado) || 0), 0);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (val: string, row: any) => (
        <div>
          <p className="text-sm font-medium text-gray-800 line-clamp-1">{val || 'Prefeitura'}</p>
          <p className="text-xs text-gray-500 mt-1">Data: {formatDateISO(row.data)}</p>
        </div>
      ),
    },
    {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 bg-gray-50 rounded-xl px-6 py-4 flex flex-wrap gap-6 items-center border border-gray-100 mb-4">
        <div>
          <p className="text-sm font-medium text-gray-500 mb-0.5">Total de Concessões</p>
          {loading ? (
        ...
        <div>
          ...
          {loading ? (
            ...
            <p className="text-xl font-semibold text-gray-900 tabular-nums">{formatBRL(valorTotal)}</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-6 rounded-xl border border-blue-100 bg-blue-50 px-6 py-4">
        ...
        <p className="text-sm text-blue-800/80 leading-relaxed">
          ...
          conforme legislação municipal pertinente e normativas do TCE-PI e PNTP 2026.
        </p>
      </div>
    </ContentPage>
  );
