...
import { executarImportacaoMensal } from "@/scripts/automatico/importar-pessoal-mes";
...
export async function GET(request: Request) {
  ...
  try {
    await executarImportacaoMensal();
    return NextResponse.json({ 
      success: true, 
      message: "Monthly payroll import triggered and completed successfully." 
    });
  } catch (error: any) {
    console.error("Vercel Cron error during personnel import:", error);
    return NextResponse.json({ 
      success: false, 
