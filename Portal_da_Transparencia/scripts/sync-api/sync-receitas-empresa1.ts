...
function agruparDadosApi(dadosApi: any[]) {
  const map = new Map<string, any>();

  for (const dado of dadosApi) {
    const cod = dado.CODIGO;
    if (!map.has(cod)) {
      map.set(cod, {
        ...dado,
        temConsolidador: (!dado.EMPRESA || String(dado.EMPRESA).trim() === ''),
        PREVISAO_INICIAL_NUM: parseValor(dado.PREVISAO_INICIAL),
        PREVISAO_ATUALIZADA_NUM: parseValor(dado.PREVISAO_ATUALIZADA),
        ARRECADADO_PERIODO_NUM: parseValor(dado.ARRECADADO_PERIODO),
        ARRECADADO_TOTAL_NUM: parseValor(dado.ARRECADADO_TOTAL),
        todasFontes: new Set<string>([dado.FONTE].filter(Boolean)),
        todosVinculos: new Set<string>([dado.VINCODIGO].filter(Boolean)),
        todasFontesSTN: new Set<string>([dado.FONTESTN].filter(Boolean)),
      });
    } else {
      const consolidado = map.get(cod);
      const isConsolidador = (!dado.EMPRESA || String(dado.EMPRESA).trim() === '');
      
      if (isConsolidador) {
        consolidado.temConsolidador = true;
        consolidado.PREVISAO_INICIAL_NUM = parseValor(dado.PREVISAO_INICIAL);
        consolidado.PREVISAO_ATUALIZADA_NUM = parseValor(dado.PREVISAO_ATUALIZADA);
        consolidado.ARRECADADO_PERIODO_NUM = parseValor(dado.ARRECADADO_PERIODO);
        consolidado.ARRECADADO_TOTAL_NUM = parseValor(dado.ARRECADADO_TOTAL);
      } else {
        if (!consolidado.temConsolidador) {
          consolidado.PREVISAO_INICIAL_NUM += parseValor(dado.PREVISAO_INICIAL);
          consolidado.PREVISAO_ATUALIZADA_NUM += parseValor(dado.PREVISAO_ATUALIZADA);
          consolidado.ARRECADADO_PERIODO_NUM += parseValor(dado.ARRECADADO_PERIODO);
          consolidado.ARRECADADO_TOTAL_NUM += parseValor(dado.ARRECADADO_TOTAL);
      ...
      if (dado.FONTE) consolidado.todasFontes.add(dado.FONTE);
      if (dado.VINCODIGO) consolidado.todosVinculos.add(dado.VINCODIGO);
      if (dado.FONTESTN) consolidado.todasFontesSTN.add(dado.FONTESTN);
    }
  }

  return Array.from(map.values()).map(item => ({
    ...item,
    PREVISAO_INICIAL: item.PREVISAO_INICIAL_NUM,
    PREVISAO_ATUALIZADA: item.PREVISAO_ATUALIZADA_NUM,
    ARRECADADO_PERIODO: item.ARRECADADO_PERIODO_NUM,
    ARRECADADO_TOTAL: item.ARRECADADO_TOTAL_NUM,
    FONTE: Array.from(item.todasFontes).join(', ').substring(0, 95),
    VINCODIGO: Array.from(item.todosVinculos).join(', ').substring(0, 45),
    FONTESTN: Array.from(item.todasFontesSTN).join(', ').substring(0, 45)
  }));
}
...
async function sincronizarEmpresa(empresa: Empresa, index: number, total: number) {
  ...
  try {
    ...
    // const dadosApi = agruparDadosApi(dadosApiRaw);
    // Removemos o agrupamento para manter todos os 134 registros originais
    ...
    for (let i = 0; i < registros.length; i += CHUNK_SIZE) {
      ...
      // Como a tabela é limpa antes, usamos apenas insert (sem onConflict)
...
async function main() {
  ...
  try {
    ...
    const empresa1 = empresas.find(e => e.codigo === '1');
    
    if (empresa1) {
      await sincronizarEmpresa(empresa1, 1, 1);
    } else {
