/**
 * PAINEL ADMIN: e-SIC — Gestão de Solicitações
 * Usa o layout admin existente (sidebar).
 *
 * @module app/admin/esic/page
 */
"use client";
...
import type { EsicSolicitacao, EsicStatus } from "@/lib/types/esic";
import {
  Search, Filter, FileText, ChevronLeft, ChevronRight,
  Loader2, Inbox, Download, Eye, X, Send, AlertCircle,
  Clock, MessageSquare, CheckCircle, Paperclip, UploadCloud
} from "lucide-react";

// Templates rápidos
const TEMPLATES: Record<string, string> = {
  "Recebimento": "Sua solicitação foi recebida com sucesso e encaminhada para o setor responsável. O prazo legal para resposta é de até 20 dias.",
  "Em análise": "Informamos que o seu pedido encontra-se em análise pelo setor técnico competente. Em breve retornaremos com a resposta.",
  "Prorrogação": "Comunicamos a prorrogação do prazo de resposta por mais 10 dias, conforme art. 11, §1º da Lei 12.527/2011, devido à complexidade da informação solicitada.",
  "Encaminhamento": "Segue em anexo/abaixo a informação solicitada, em atendimento à Lei de Acesso à Informação.",
  "Indeferimento": "Informamos que o seu pedido foi indeferido. Conforme o Art. 15 da LAI, você tem o direito de apresentar recurso no prazo de 10 dias."
};
...
function getPrazoStatus(prazoIso: string, status: EsicStatus) {
...
export default function AdminEsicPage() {
  const [items, setItems] = useState<EsicSolicitacao[]>([]);
  ...
  const [sel, setSel] = useState<EsicSolicitacao | null>(null);
  const [modal, setModal] = useState(false);
  const [toast, setToast] = useState<ToastData | null>(null);
  const [saving, setSaving] = useState(false);
  const [mStatus, setMStatus] = useState<EsicStatus>("em_analise");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/esic?${p}`);
  ...
  useEffect(() => { setPage(1); }, [statusFilter, debounced]);

  function openModal(item: EsicSolicitacao) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch("/api/esic", { method: "PATCH", body });
  ...
  async function handleExport() {
    try {
      ...
      if (statusFilter !== "todos") p.set("status", statusFilter);
      if (debounced) p.set("search", debounced);
      const { data }: { data: EsicSolicitacao[] } = await (await fetch(`/api/esic?${p}`)).json();
      if (!data?.length) { setToast({ type: "info", message: "Sem dados." }); return; }
      const h = ["Protocolo", "Nome", "E-mail", "Status", "Data", "Prazo", "Respondido"];
      const rows = data.map(s => [s.protocolo, s.nome, s.email, s.status, fmtDate(s.created_at), fmtDate(s.prazo_resposta), s.respondido_em ? fmtDate(s.respondido_em) : "-"]);
      const csv = [h.join(";"), ...rows.map(r => r.join(";"))].join("\n");
      const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url; a.download = `esic-${new Date().toISOString().split("T")[0]}.csv`; a.click(); URL.revokeObjectURL(url);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-bold text-gray-900">e-SIC</h1><p className="text-sm text-gray-500">{total} solicitação(ões)</p></div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5"><DashboardStats apiUrl="/api/esic/stats" title="Métricas" canal="esic" /></div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16"><div className="mx-auto w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center mb-4"><Inbox size={28} className="text-gray-400" /></div><h4 className="text-gray-600 font-semibold">Nenhuma solicitação</h4></div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left px-5 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest">Solicitante</th>
                ...
                <tbody>{items.map(s => {
                  const prazoObj = getPrazoStatus(s.prazo_resposta, s.status);
                  return (
                    <tr key={s.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                      <td className="px-5 py-4"><span className="font-bold text-[#0B3D91] tracking-wider text-xs">{s.protocolo}</span></td>
                      <td className="px-5 py-4"><p className="font-medium text-gray-900">{s.nome}</p><p className="text-xs text-gray-400">{s.email}</p></td>
                      <td className="px-5 py-4"><StatusBadge status={s.status} canal="esic" size="sm" /></td>
                      <td className="px-5 py-4 text-gray-500 text-xs">{fmtDate(s.created_at)}</td>
                      <td className="px-5 py-4">
                        <div className={`inline-flex px-2 py-1 rounded-md border text-xs font-bold ${prazoObj.bg} ${prazoObj.color}`}>
                          {prazoObj.label}
                        </div>
                        <div className="text-[10px] text-gray-400 mt-0.5 ml-1">{fmtDate(s.prazo_resposta)}</div>
                      </td>
                      <td className="px-5 py-4 text-right"><button onClick={() => openModal(s)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#0B3D91] text-white rounded-lg text-xs font-bold hover:bg-[#082a64]"><Eye size={14} /> Ver</button></td>
                    </tr>
                  )
                })}</tbody>
              </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b p-5 flex items-center justify-between rounded-t-2xl z-10">
              <div><p className="text-xs font-black text-gray-500 uppercase tracking-widest mb-1">Protocolo</p><p className="text-xl font-black text-[#0B3D91] tracking-wider">{sel.protocolo}</p></div>
              <button onClick={() => setModal(false)} className="p-2 hover:bg-gray-100 rounded-lg"><X size={20} className="text-gray-500" /></button>
            </div>
            <div className="p-5 space-y-5">
              <div className="bg-gray-100/80 rounded-xl p-4 border border-gray-200"><p className="text-xs font-black text-[#0B3D91] uppercase tracking-widest mb-3">Solicitante</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-900 font-semibold"><p><strong className="text-gray-500 font-normal">Nome:</strong><br />{sel.nome}</p><p><strong className="text-gray-500 font-normal">E-mail:</strong><br />{sel.email}</p>{sel.cpf && <p><strong className="text-gray-500 font-normal">CPF/CNPJ:</strong><br />{sel.cpf}</p>}{sel.telefone && <p><strong className="text-gray-500 font-normal">Tel:</strong><br />{sel.telefone}</p>}</div></div>
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4"><div className="flex items-center gap-2 mb-3"><FileText size={16} className="text-blue-700" /><p className="text-xs font-black text-blue-800 uppercase tracking-widest">Solicitação</p></div><p className="text-sm text-blue-900 whitespace-pre-wrap leading-relaxed">{sel.descricao}</p></div>
