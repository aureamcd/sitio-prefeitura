/**
 * API: Consulta pública de protocolo e-SIC
 * GET /api/esic/consultar?protocolo=ESIC-2026-00001&email=xxx
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("[e-SIC Consulta] DB error:", error);
    ...
    if (!emailMatch && !cpfMatch) {
      return NextResponse.json(
        { error: "Os dados não correspondem ao protocolo." },
        { status: 403 }
      );
    }

    /* Retorna sem dados sensíveis (email/cpf ficam no servidor) */
    return NextResponse.json({
    ...
    console.error("[e-SIC Consulta] Erro:", err);
